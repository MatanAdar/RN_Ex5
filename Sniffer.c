...
if(file==NULL)
{
  ...
  { // 0x0800 is IP type
    ...
    // struct tcphdr* tcp_header = (struct tcphdr*) (packet + sizeof(struct ethhdr) + sizeof(struct ipheader));
    // unsigned int timestamp = tcp_header->th_timestamp;
    // time_t epoch_time = (time_t) timestamp;
